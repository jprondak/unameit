...
# 02111-1307, USA.
#
# $Id: pull_main.tcl,v 1.107.2.1 1997/06/17 21:12:08 simpson Exp $
#
...
proc post_process_move {type {region ""}} {
    ...
    global DNS_REGIONS GEN_FILES MY_CNAME
	...
	if {[cequal $type dns] && [array size DNS_REGIONS] > 0} {
	    if {[catch {open [file join /etc named.pid] r} fd] == 0} {
		gets $fd line
		if {![regexp {[0-9]+} $line]} {
		    puts stderr "[file join /etc named.pid]\
			    file doesn't contain a pid"
		} else {
		    catch {kill HUP $line}
		}
		close $fd
	...
	if {[cequal $type etc] &&
	[info exists GEN_FILES(dhcp.host.$MY_CNAME)]} {
	    if {[catch {open [file join /etc dhcpd.pid] r} fd] == 0} {
		gets $fd line
		if {![regexp {[0-9]+} $line]} {
		    puts stderr "[file join /etc dhcpd.pid]\
			    file doesn't contain a pid"
		} else {
		    catch {kill $line}
		    if {[catch {exec dhcpd 2>/dev/null} result]} {
			puts stderr $result
		    }
		}
		close $fd
...
proc process_dhcp {} {
    global GEN_FILES MY_CNAME VERSION UNAMEIT_DATA FILE_LIST

    if {[info exists GEN_FILES(dhcp.host.$MY_CNAME)]} {
	unameit_cp [file join gen.$VERSION dhcp.host.$MY_CNAME]\
		[file join $UNAMEIT_DATA $VERSION etc dhcpd.conf]
	lappend FILE_LIST(etc) [file join $UNAMEIT_DATA $VERSION etc\
		dhcpd.conf]
    }
}
...
proc main {} {
    ...
    global OS RELEASE SHORT_HOST MY_NAME INET_DIR MAIL_DIR USE_PLUSES
